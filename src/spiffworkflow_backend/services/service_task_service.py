...
class CustomServiceTask(ServiceTask):  # type: ignore
    def _execute(self, spiff_task: SpiffTask) -> bool:
        def evaluate(param) -> dict:
            if isinstance(param, dict):
                # Если param это словарь, выполняем стандартное действие
                param["value"] = spiff_task.workflow.script_engine.evaluate(spiff_task, param["value"])
                return param
            elif isinstance(param, str):
                # Если param это строка, обработайте её как нужно (например, можно просто вернуть)
                return {"value": spiff_task.workflow.script_engine.evaluate(spiff_task, param)}
            else:
                # Если param не строка и не словарь, можно поднять исключение или обработать по-другому
                raise ValueError(f"Unsupported type for param: {type(param)}")
